...
const INDEXEDDB_CONFIG = {
    ...
    TTL: {
        ...
        USER_PROFILES: 24 * 60 * 60 * 1000, // 24 hours
        GROUP_DATA: 12 * 60 * 60 * 1000, // 12 hours
        GROUP_MEMBERS: 6 * 60 * 60 * 1000, // 6 hours
        PRIVATE_CHATS: 7 * 24 * 60 * 60 * 1000 // 7 days
    }
};
...
class IndexedDBCache {
    ...
    async init() {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains(INDEXEDDB_CONFIG.STORES.MESSAGES)) {
                    const messageStore = db.createObjectStore(INDEXEDDB_CONFIG.STORES.MESSAGES, { keyPath: ['groupId', 'id'] });
                    messageStore.createIndex('groupId', 'groupId', { unique: false });
                    messageStore.createIndex('timestamp', 'timestamp', { unique: false });
                }
                
                if (!db.objectStoreNames.contains(INDEXEDDB_CONFIG.STORES.USER_PROFILES)) {
                    const userStore = db.createObjectStore(INDEXEDDB_CONFIG.STORES.USER_PROFILES, { keyPath: 'id' });
                    userStore.createIndex('lastUpdated', 'lastUpdated', { unique: false });
                }
                
                if (!db.objectStoreNames.contains(INDEXEDDB_CONFIG.STORES.GROUP_DATA)) {
                    const groupStore = db.createObjectStore(INDEXEDDB_CONFIG.STORES.GROUP_DATA, { keyPath: 'id' });
                    groupStore.createIndex('lastUpdated', 'lastUpdated', { unique: false });
                }
                
                if (!db.objectStoreNames.contains(INDEXEDDB_CONFIG.STORES.GROUP_MEMBERS)) {
                    const membersStore = db.createObjectStore(INDEXEDDB_CONFIG.STORES.GROUP_MEMBERS, { keyPath: ['groupId', 'id'] });
                    membersStore.createIndex('groupId', 'groupId', { unique: false });
                    membersStore.createIndex('lastUpdated', 'lastUpdated', { unique: false });
                }
                
                if (!db.objectStoreNames.contains(INDEXEDDB_CONFIG.STORES.PRIVATE_CHATS)) {
                    const privateStore = db.createObjectStore(INDEXEDDB_CONFIG.STORES.PRIVATE_CHATS, { keyPath: ['chatId', 'id'] });
                    privateStore.createIndex('chatId', 'chatId', { unique: false });
                    privateStore.createIndex('timestamp', 'timestamp', { unique: false });
                }
                
                if (!db.objectStoreNames.contains(INDEXEDDB_CONFIG.STORES.SETTINGS)) {
                    db.createObjectStore(INDEXEDDB_CONFIG.STORES.SETTINGS, { keyPath: 'key' });
                }
            };
        });
    }
    ...
    async storeMessages(groupId, messages) {
        try {
            await this.waitForInit();
            const transaction = this.db.transaction([INDEXEDDB_CONFIG.STORES.MESSAGES], 'readwrite');
            const store = transaction.objectStore(INDEXEDDB_CONFIG.STORES.MESSAGES);
            
            // Store each message with groupId and TTL
            const now = Date.now();
            for (const message of messages) {
                const item = {
                    ...
                    groupId: groupId,
                    cachedAt: now,
                    expiresAt: now + INDEXEDDB_CONFIG.TTL.MESSAGES
                };
                await store.put(item);
            }
            
            await transaction.complete;
            console.log(`Stored ${messages.length} messages for group ${groupId} in IndexedDB`);
        } catch (error) {
    ...
    async getMessages(groupId, limit = 50) {
        try {
            await this.waitForInit();
            const transaction = this.db.transaction([INDEXEDDB_CONFIG.STORES.MESSAGES], 'readonly');
            const store = transaction.objectStore(INDEXEDDB_CONFIG.STORES.MESSAGES);
    ...
    async storeUserProfile(userId, profile) {
        try {
            await this.waitForInit();
            const transaction = this.db.transaction([INDEXEDDB_CONFIG.STORES.USER_PROFILES], 'readwrite');
            const store = transaction.objectStore(INDEXEDDB_CONFIG.STORES.USER_PROFILES);
            
            const item = {
                ...profile,
                id: userId,
                lastUpdated: Date.now(),
                expiresAt: Date.now() + INDEXEDDB_CONFIG.TTL.USER_PROFILES
            };
            
            await store.put(item);
            await transaction.complete;
        } catch (error) {
            console.error('Error storing user profile in IndexedDB:', error);
        }
    }
    ...
    async getUserProfile(userId) {
        try {
            await this.waitForInit();
            const transaction = this.db.transaction([INDEXEDDB_CONFIG.STORES.USER_PROFILES], 'readonly');
            const store = transaction.objectStore(INDEXEDDB_CONFIG.STORES.USER_PROFILES);
    ...
    async storeGroupData(groupId, groupData) {
        try {
            await this.waitForInit();
            const transaction = this.db.transaction([INDEXEDDB_CONFIG.STORES.GROUP_DATA], 'readwrite');
            const store = transaction.objectStore(INDEXEDDB_CONFIG.STORES.GROUP_DATA);
            
            const item = {
                ...groupData,
                id: groupId,
                lastUpdated: Date.now(),
                expiresAt: Date.now() + INDEXEDDB_CONFIG.TTL.GROUP_DATA
            };
            
            await store.put(item);
            await transaction.complete;
        } catch (error) {
            console.error('Error storing group data in IndexedDB:', error);
        }
    }
    ...
    async getGroupData(groupId) {
        try {
            await this.waitForInit();
            const transaction = this.db.transaction([INDEXEDDB_CONFIG.STORES.GROUP_DATA], 'readonly');
            const store = transaction.objectStore(INDEXEDDB_CONFIG.STORES.GROUP_DATA);
            ...
            return new Promise((resolve, reject) => {
                request.onsuccess = () => {
                    const groupData = request.result;
                    if (groupData && groupData.expiresAt > Date.now()) {
                        resolve(groupData);
                    } else {
                        if (groupData) {
            ...
            console.error('Error getting group data from IndexedDB:', error);
            return null;
        }
    }
    ...
    async clearOldData() {
        try {
            ...
            await this.clearExpiredData(INDEXEDDB_CONFIG.STORES.MESSAGES, 'expiresAt', now);
            
            // Clear expired user profiles
            await this.clearExpiredData(INDEXEDDB_CONFIG.STORES.USER_PROFILES, 'expiresAt', now);
            
            // Clear expired group data
            await this.clearExpiredData(INDEXEDDB_CONFIG.STORES.GROUP_DATA, 'expiresAt', now);
            
            // Clear expired group members
            await this.clearExpiredData(INDEXEDDB_CONFIG.STORES.GROUP_MEMBERS, 'expiresAt', now);
            
            // Clear expired private chats
            await this.clearExpiredData(INDEXEDDB_CONFIG.STORES.PRIVATE_CHATS, 'expiresAt', now);
            
            console.log('Cleaned up expired IndexedDB data');
        } catch (error) {
            console.error('Error clearing old IndexedDB data:', error);
        }
    }

    async clearExpiredData(storeName, expiryField, now) {
        const transaction = this.db.transaction([storeName], 'readwrite');
        const store = transaction.objectStore(storeName);
        const index = store.index(expiryField);
    ...
    async clearAll() {
        try {
            ...
            const transaction = this.db.transaction([
                INDEXEDDB_CONFIG.STORES.MESSAGES,
                INDEXEDDB_CONFIG.STORES.USER_PROFILES,
                INDEXEDDB_CONFIG.STORES.GROUP_DATA,
                INDEXEDDB_CONFIG.STORES.GROUP_MEMBERS,
                INDEXEDDB_CONFIG.STORES.PRIVATE_CHATS
            ], 'readwrite');
            
            transaction.objectStore(INDEXEDDB_CONFIG.STORES.MESSAGES).clear();
            transaction.objectStore(INDEXEDDB_CONFIG.STORES.USER_PROFILES).clear();
            transaction.objectStore(INDEXEDDB_CONFIG.STORES.GROUP_DATA).clear();
            transaction.objectStore(INDEXEDDB_CONFIG.STORES.GROUP_MEMBERS).clear();
            transaction.objectStore(INDEXEDDB_CONFIG.STORES.PRIVATE_CHATS).clear();
...
class ServiceWorkerManager {
    ...
    async syncOfflineData() {
        if (!this.isSupported || !this.registration || !this.registration.sync) {
            return false;
        }
    ...
    async cacheData(url, data) {
        ...
        try {
            ...
            console.error('Error caching data via Service Worker:', error);
            return false;
        }
    }
    ...
    async getCachedData(url) {
        ...
        try {
            ...
            console.error('Error getting cached data from Service Worker:', error);
            return null;
        }
    }
}
...
class GroupChat {
    ...
    async initServiceWorker() {
        if (this.serviceWorkerManager.isSupported) {
            ...
            // Send app configuration to Service Worker
    ...
    // NEW: Handle messages from Service Worker
    handleServiceWorkerMessage(message) {
        switch (message.type) {
            case 'NETWORK_STATUS':
                this.isOnline = message.online;
                this.connectionState = message.online ? 'connected' : 'disconnected';
                console.log('Network status from Service Worker:', this.isOnline);
                break;
            ...
            case 'CACHE_UPDATED':
                console.log('Cache updated:', message.cacheName);
                break;
                
            case 'OFFLINE_DATA_SYNCED':
                this.handleOfflineDataSynced(message.data);
                break;
        }
    }

    // NEW: Handle offline data that was synced
    async handleOfflineDataSynced(data) {
        if (data.type === 'messages') {
            // Messages were synced, we might want to refresh
            if (this.currentGroupId === data.groupId) {
                console.log('Offline messages synced for current group');
    ...
    // NEW: Setup IndexedDB cleanup interval
    setupIndexedDBCleanup() {
        // Clean up old data every hour
        setInterval(() => {
            this.indexedDBCache.clearOldData();
        }, 60 * 60 * 1000);
    }
    ...
    // UPDATED: Get cached item with IndexedDB fallback
    async getCachedItem(cacheKey, cacheMap, indexedDBStore = null, forceRefresh = false) {
        ...
        // If not in memory cache, check IndexedDB if store is specified
        if (indexedDBStore && !forceRefresh) {
            try {
                let indexedDBData = null;
                
                switch (indexedDBStore) {
                    case 'userProfiles':
                        if (cacheKey.startsWith('user_')) {
                            const userId = cacheKey.replace('user_', '');
                            indexedDBData = await this.indexedDBCache.getUserProfile(userId);
                        }
                        break;
                        
                    case 'groupData':
                        indexedDBData = await this.indexedDBCache.getGroupData(cacheKey);
                        break;
                        
                    case 'messages':
                        if (cacheKey.startsWith('messages_')) {
                            const parts = cacheKey.split('_');
                            if (parts.length >= 2) {
                                const groupId = parts[1];
                                const limit = parts.length > 2 ? parseInt(parts[2]) : 50;
                                indexedDBData = await this.indexedDBCache.getMessages(groupId, limit);
                            }
                        }
                        break;
                }
                ...
                if (indexedDBData) {
                    ...
                    this.setCachedItem(cacheKey, indexedDBData, cacheMap, CACHE_DURATION.GROUP_DATA);
    ...
    // UPDATED: Set cached item with IndexedDB persistence
    async setCachedItem(cacheKey, data, cacheMap, duration, indexedDBStore = null) {
        ...
        // Also store in IndexedDB if store is specified
        if (indexedDBStore) {
            try {
                switch (indexedDBStore) {
                    case 'userProfiles':
                        if (cacheKey.startsWith('user_')) {
                            const userId = cacheKey.replace('user_', '');
                            await this.indexedDBCache.storeUserProfile(userId, data);
                        }
                        break;
                        
                    case 'groupData':
                        await this.indexedDBCache.storeGroupData(cacheKey, data);
                        break;
                    ...
                    case 'messages':
                        if (cacheKey.startsWith('messages_')) {
                            ...
                            if (parts.length >= 2) {
                                const groupId = parts[1];
                                await this.indexedDBCache.storeMessages(groupId, data);
                            }
                        }
                        break;
                }
    ...
    clearGroupCache(groupId) {
        ...
        this.indexedDBCache.clearExpiredData(
            INDEXEDDB_CONFIG.STORES.MESSAGES,
            'groupId',
            groupId
        ).catch(console.error);
    ...
    async clearAllCache() {
        ...
        // Clear IndexedDB
        await this.indexedDBCache.clearAll();
    ...
    // UPDATED: Get user profile with IndexedDB caching
    async getUserProfile(userId, forceRefresh = false) {
        try {
            const cacheKey = `user_${userId}`;
            
            // First check memory and IndexedDB cache
            if (!forceRefresh) {
    ...
    async updateUserReward(userId, rewardData) {
        try {
            ...
            // Update IndexedDB
            await this.indexedDBCache.storeUserProfile(userId, cached || {
                id: userId,
                rewardTag: rewardData.tag,
                glowEffect: rewardData.glowEffect,
                fireRing: rewardData.fireRing
    ...
    async updateUserProfile(userData) {
        try {
            ...
            // Update IndexedDB
            await this.indexedDBCache.storeUserProfile(this.firebaseUser.uid, this.currentUser);
    ...
    async createGroup(groupData, photoFile = null) {
        try {
            ...
            this.setCachedItem(groupRef.id, group, this.cache.groupData, CACHE_DURATION.GROUP_DATA, 'groupData');
    ...
    async getAllGroups() {
        try {
            ...
            querySnapshot.forEach(doc => {
                ...
                this.setCachedItem(doc.id, group, this.cache.groupData, CACHE_DURATION.GROUP_DATA, 'groupData');
            });
    ...
    // UPDATED: Get group with IndexedDB caching
    async getGroup(groupId, forceRefresh = false) {
        try {
            if (!forceRefresh) {
                ...
                const indexedDBGroup = await this.indexedDBCache.getGroupData(groupId);
                if (indexedDBGroup) {
                    this.setCachedItem(groupId, indexedDBGroup, this.cache.groupData, CACHE_DURATION.GROUP_DATA);
                    return indexedDBGroup;
                }
            }
            ...
            if (groupSnap.exists()) {
                ...
                this.setCachedItem(groupId, group, this.cache.groupData, CACHE_DURATION.GROUP_DATA, 'groupData');
    ...
    // UPDATED: Get messages with IndexedDB caching
    async getMessages(groupId, limitCount = 50) {
        try {
            ...
            if (cached) {
                console.log('Loaded messages from cache for group:', groupId);
                return cached;
            }
    ...
    // UPDATED: listenToMessages method with IndexedDB caching
                ...
                try {
                    ...
                    if (messages.length > 0) {
                        ...
                        if (isFirstSnapshot) {
                            ...
                            // Store initial messages in IndexedDB
                            await this.indexedDBCache.storeMessages(groupId, messages);
                        } else {
...
function initGroupPage() {
    ...
    async function loadGroupData() {
        try {
            // Try to load group data from cache first
            groupData = await groupChat.getGroup(groupId, false);
            
            if (!groupData) {
                // If not in cache, load from Firebase
                groupData = await groupChat.getGroup(groupId, true);
            ...
            if (isInitialLoad) {
                // Try to load cached messages first for faster display
                const cachedMessages = await groupChat.indexedDBCache.getMessages(groupId, 50);
                if (cachedMessages && cachedMessages.length > 0) {
                    ...
                    console.log('No cached messages found in IndexedDB');
                }
                isInitialLoad = false;
            }
    ...
    function setupListeners() {
        ...
        const messagesUnsubscribe = groupChat.listenToMessages(groupId, (newMessages) => {
            ...
            // Clear messages array on first load if no cached messages
            if (messages.length === 0) {
