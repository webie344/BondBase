...
/* =================================================================
   ============================================================= ====
   MERGED FEATURES (formerly features.js)
   - Chat customization (per-chat theme + wallpaper)
   - Songs on drops
   - Reply drops (photo replies)
   - Streak shields
   - Monthly recap
   Wrapped in an IIFE so it can never blank the host page —
   any error inside is caught and logged.
   ============================================================= ====
   ================================================================= */
(function setupDropFeatures() {
    try {
        const App = window.dropApp;
        if (!App) { console.warn("[features] window.dropApp missing"); return; }
        const { state, db, $, $$, escapeHtml, showToast, todayKey, uploadToCloudinary } = App;
        const F = App.firestore;
        const extractHashtags = App.extractHashtags || ((s) =>
            (String(s || "").match(/#([\p{L}\p{N}_]+)/gu) || []).map(t => t.slice(1).toLowerCase())
        );
        ...
        // Wait until Firebase auth has resolved (state.user populated)
        const waitForUser = () => new Promise(resolve => {
            if (state.user?.uid) return resolve(state.user);
            const poll = setInterval(() => {
                if (state.user?.uid) { clearInterval(poll); resolve(state.user); }
            }, 250);
        });
            ...
            save(chatId, prefs) {
                ...
                if (me) {
                    F.setDoc(F.doc(db, "users", me, "chatPrefs", chatId), {
                        ...prefs, updatedAt: F.serverTimestamp()
                    }, { merge: true }).catch(() => {});
                }
            },
            apply(prefs) {
        ...
        const Songs = {
            pendingSong: null, activeAudio: null, activeBadge: null, activeRow: null,
            pendingKey: "drop:pending-song",
            loadPending() {
                try { const raw = localStorage.getItem(this.pendingKey); if (raw) this.pendingSong = JSON.parse(raw); } catch {}
            },
            savePending() {
                try {
            ...
            injectAddPill() {
                ...
                if (previewBlock.querySelector(".add-song-wrap")) { this.refreshAddPill(); return; }
                const wrap = document.createElement("div");
            ...
            refreshAddPill() {
                ...
                if (pill) { pill.innerHTML = this.pillHTML(); pill.classList.toggle("has-song", !!this.pendingSong); }
                if (clear) clear.hidden = !this.pendingSong;
            },
            activeMood: "all", activeQuery: "",
            openPicker() { this.renderPickerList(); openDialog("song-picker-dialog"); },
            filteredSongs() {
            ...
            renderPickerList() {
                ...
                if (!songs.length) { list.innerHTML = `<li class="recap-empty">No songs match. Try another mood.</li>`; return; }
                ...
                list.querySelectorAll(".song-row").forEach(row => {
                    ...
                    row.addEventListener("click", (e) => {
                        if (e.target.closest('[data-action="play"]')) { this.previewSong(song, row); return; }
            ...
            selectSong(song) { this.pendingSong = song; this.savePending(); this.refreshAddPill(); },
            previewSong(song, row) {
                if (this.activeAudio && this.activeRow === row) { this.stopActive(); return; }
                this.stopActive();
            ...
            stopActive() {
                if (this.activeAudio) { try { this.activeAudio.pause(); } catch {} this.activeAudio = null; }
            ...
            enhancePostCard(card) {
                ...
                this.fetchPostSong(postId).then(song => {
                    ...
                    badge.innerHTML = `<span class="psb-disc"></span><span class="psb-text"><strong>${escapeHtml(song.title)}</strong>${escapeHtml(song.artist)}</span>`;
                    badge.addEventListener("click", (e) => { e.stopPropagation(); this.toggleBadgePlayback(badge, song); });
                    wrap.appendChild(badge);
                }).catch(() => {});
            },
            toggleBadgePlayback(badge, song) {
                if (this.activeAudio && this.activeBadge === badge) { this.stopActive(); return; }
                this.stopActive();
                const audio = new Audio(song.url);
                audio.crossOrigin = "anonymous";
                audio.play().then(() => {
            ...
            watchOwnNewPosts() {
                ...
                const q = F.query(F.collection(db, "posts"), F.where("uid", "==", me), F.orderBy("createdAt", "desc"), F.limit(1));
                F.onSnapshot(q, (snap) => {
        ...
        /* ===== 3. REPLY DROPS ===== */
        const ReplyDrops = {
            pendingParentId: null, pendingParentData: null, pendingFile: null,
            enhancePostCard(card) {
                ...
                if (actions && !actions.querySelector(".reply-drop-btn")) {
                    ...
                    btn.addEventListener("click", (e) => { e.stopPropagation(); this.openComposer(postId); });
            ...
            onPickFile(file) {
                if (!file) return;
                if (file.size > 12 * 1024 * 1024) { showToast("Photo is over 12MB.", "error"); return; }
            ...
            setupComposer() {
                ...
                if (caption && count) caption.oninput = (e) => { count.textContent = `${e.target.value.length} / 240`; };
            ...
            async submit() {
                ...
                try {
                    ...
                    const newDoc = {
                        ...
                        likes: 0, likedBy: [], commentsCount: 0, viewsCount: 0, reactions: {}
                    };
        ...
        const Shields = {
            ...
            render() {
                const targets = document.querySelectorAll("[data-streak-pill], .streak-pill, .streak-display, #streak-count, .home-streak");
        ...
        const Recap = {
            ...
            async fetchMonth(offset = 0) {
                ...
                try {
                    const q = F.query(F.collection(db, "posts"),
                        F.where("uid", "==", me),
                        F.where("promptDate", ">=", startKey),
                        F.where("promptDate", "<=", endKey));
                    const snap = await F.getDocs(q);
                    const posts = []; let totalLikes = 0;
                    snap.forEach(d => { const data = d.data() || {}; posts.push({ id: d.id, ...data }); totalLikes += data.likes || 0; });
                    return { posts, totalLikes };
                } catch (e) { return { posts: [], totalLikes: 0 }; }
            },
            async render() {
                ...
                if (!posts.length) { stage.innerHTML = `<div class="recap-empty">No drops in ${escapeHtml(label)}.</div>`; return; }
                ...
                for (let i = 0; i < 9; i++) {
                    ...
                    } else cells.push(`<div class="rc-cell"></div>`);
                }
                stage.innerHTML = `
                    <div class="recap-card" id="recap-card-render">
                        <div class="rc-header"><span class="rc-month">${escapeHtml(label)}</span><span class="rc-brand">DROP</span></div>
            ...
            async saveImage() {
                ...
                if (!card || typeof window.html2canvas !== "function") { showToast("Image rendering not available.", "error"); return; }
                try {
                    ...
                    showToast("Saved!");
                } catch (e) { console.error(e); showToast("Couldn't save image.", "error"); }
            },
            injectTrigger() {
                ...
                candidates.forEach(target => {
                    ...
                    btn.innerHTML = `<span class="rt-icon">✨</span><span class="rt-meta"><span class="rt-title">Your month in drops</span><span class="rt-sub">A shareable recap of this month</span></span>`;
                    btn.addEventListener("click", () => { this.monthOffset = 0; this.render(); openDialog("monthly-recap-dialog"); });
                    target.prepend(btn);
                });
            },
            init() {
        ...
        console.log("[features] merged into app.js — ready");
    } catch (err) {
        console.error("[features] fatal setup error", err);
    }
})();
