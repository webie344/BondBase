        ...
        try {
            ...
            if (videoUrl.includes('/upload/')) {
                ...
                const thumb = videoUrl
                    ...
                    .replace(/\.(mp4|mov|avi|mkv|webm|3gp|m4v|flv|wmv)$/i, '.jpg');
                return thumb;
            }
...
class TikTokFeed {
    ...
    _positionSlides(centreIndex, dragOffset = 0) {
        this.videos.forEach((video, idx) => {
            ...
            // Never hide slides with display:none — translateY already keeps them off-screen.
            // display:none breaks CSS transitions causing videos to "skip" when swiping back.
                    ...
                    <div class="side-action-icon">
                        <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path class="heart-outline" d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5
                                ...
                                17.5 3 20.58 3 23 5.42 23 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                        </svg>
                    </div>
    ...
    async loadAndPlayVideo(videoId) {
        ...
        // ── Helper: show video and play together so audio never plays on a blank screen
        const revealAndPlay = () => {
            if (loadingElement) loadingElement.style.display = 'none';
            videoElement.style.display = 'block';
            // Only play if this is still the current video
            const currentId = this.videos[this.currentIndex]?.id;
            if (currentId === videoId) {
                const p = videoElement.play();
                if (p) p.catch(err => console.log('Autoplay blocked:', err));
            }
        };

        if (!videoElement.dataset.listenersAdded) {
            // First frame ready — reveal video and start playback together
            videoElement.addEventListener('canplay', () => {
                revealAndPlay();
            });

            // Backup: loadeddata also works
            videoElement.addEventListener('loadeddata', () => {
                revealAndPlay();
            });

            // Mid-playback rebuffer — show spinner again
            videoElement.addEventListener('waiting', () => {
                // Small delay to avoid flickering on brief stalls
                const waitId = setTimeout(() => {
                    if (videoElement.readyState < 3 && loadingElement) {
                        videoElement.style.display = 'none'; // hide frozen frame
                        loadingElement.style.display = 'block';
                    }
                }, 300);
                // Cancel if it recovers quickly
                videoElement.addEventListener('playing', () => clearTimeout(waitId), { once: true });
            });
                ...
                if (loadingElement) {
                    loadingElement.innerHTML = `
                        <div style="text-align:center;">
                            ...
                            <div style="margin-bottom:12px;font-size:14px;">Failed to load video</div>
                            <button onclick="tiktokFeedInstance.retryVideo('${videoId}')"
                                    style="padding:8px 20px;background:#ff2d55;border:none;border-radius:20px;color:white;cursor:pointer;font-size:13px;font-weight:600;">
                                Retry
                            </button>
                        </div>
                    `;
        ...
        // ── If already buffered: reveal immediately, no spinner needed
        if (videoElement.readyState >= 3) {
            revealAndPlay();
            return;
        }

        // ── Not buffered yet: hide the video frame and show the spinner immediately
    ...
    playCurrentVideo() {
        if (this.currentIndex >= 0 && this.currentIndex < this.videos.length) {
            ...
            if (videoData && videoData.videoElement) {
                const vid = videoData.videoElement;
                // Don't play if the video element is still hidden — audio would play over blank screen
                if (vid.style.display === 'none') return;
                const p = vid.play();
                if (p) p.catch(err => console.log('Auto-play prevented:', err));
            }
        }
    }
        ...
        try {
            ...
            if (result) {
                ...
                if (button.classList.contains('liked')) {
                    ...
                    if (likeCount) {
                        likeCount.textContent = formatSocialCount(result?.likes ?? (parseInt(likeCount.textContent) || 0) + 1);
                    }
                }
            }
        } catch (error) {
...
// Inject professional TikTok-style side-action button CSS
(function injectSideActionStyles() {
    if (document.getElementById('bb-side-action-styles')) return;
    const style = document.createElement('style');
    style.id = 'bb-side-action-styles';
    style.textContent = `
        /* ── Loading overlay — covers the slide while video buffers ── */
        .video-slide {
        ...
        /* No longer needed for visibility — transform handles it */
        .video-slide.active {}

        /* Overlay always in DOM, JS toggles display between flex/none */
        .video-loading {
            position: absolute;
            inset: 0;
            display: flex;
            align-items: center;
            justify-content: center;
            /* Dark overlay so poster shows through but spinner is clearly visible */
        ...
        .video-loading-spinner {
            ...
            border-radius: 50%;
            animation: bb-spin 0.75s cubic-bezier(0.4, 0, 0.2, 1) infinite;
        ...
        .like-btn.liked .side-action-icon {
            background: rgba(255, 48, 88, 0.25);
        }

        .like-btn.liked .side-action-icon svg .heart-outline {
            fill: #ff3058;
            stroke: #ff3058;
        }
